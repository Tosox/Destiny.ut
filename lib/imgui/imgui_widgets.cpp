...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, float power)
{
    ...
    /* //Modified: TosoxDev
    if (temp_input_is_active || temp_input_start) // TosoxDev
        return TempInputScalar(frame_bb, id, label, data_type, p_data, format);
    */
    ...
    // Render grab //Modified: TosoxDev
    const float grab_padding = 2.0f;
    if (grab_bb.Max.x > grab_bb.Min.x)
    {
        ImRect new_bb = frame_bb;
        new_bb.Min.x += grab_padding;
        new_bb.Min.y += grab_bb.Min.y - frame_bb.Min.y;
        window->DrawList->AddRectFilled(new_bb.Min, grab_bb.Max, GetColorU32(g.ActiveId == id ? ImGuiCol_SliderGrabActive : ImGuiCol_SliderGrab), style.GrabRounding);
    }
